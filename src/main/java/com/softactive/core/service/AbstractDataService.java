...
import com.softactive.core.utils.DataServiceUtils;
...
import com.softactive.core.utils.MultiJdbcFilter.Condition;
import com.softactive.core.utils.SimpleJdbcFilter;
import com.softactive.core.utils.SimpleJdbcFilter.Relation;
...
public abstract class AbstractDataService<T extends Base> implements MyConstants {
	...
	protected AbstractDataService(String tableName) {
	...
	protected List<T> listOfObject(List<String> orderByFields) {
		return query(initQuery() + " order by " + addOrderBy(orderByFields));
	}

	protected List<T> listOfObject() {
	...
	protected T mapResult(ResultSet rs) throws SQLException {
		...
		for (FieldsDefinition fd : fieldsDefinitionList) {
			setTreeValue(rs, fd, t, getTableName());
		}
		return t;
	}
				...
				if (fd.getJdbcColumn().joinField().isEmpty()) {
					...
					for (FieldsDefinition childFd : fd.getFieldsDefinitionList()) {
						setTreeValue(rs, childFd, t,
								dataServiceUtils.getDataServiceFromJdbcObject(fd.getField().getType()).getTableName());
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
...
//	}
	...
	private List<T> getObjectsFromDb(List<T> list, int batchSize) {
		...
		int count = 0;
		for (T t : list) {
			multiFilterList.add(new MultiJdbcFilter(Condition.AND, createSimpleFilterArrayFromObject(t)));
			count++;
			if (count % batchSize == 0) {
				returnList.addAll(queryResultList(new MultiJdbcFilter(Condition.OR,
	...
	private SimpleJdbcFilter[] createSimpleFilterArrayFromObject(T t) {
		...
		for (FieldsDefinition def : getKeyDefinitionListJoined()) {
			...
			simpleFilterArray[i++] = new SimpleJdbcFilter(def.getJdbcColumn().field(), Relation.EQ,
	...
	protected SimpleJdbcInsert getSimpleJdbcInsert() {
		if (simpleJdbcInsert == null) {
			simpleJdbcInsert = new SimpleJdbcInsert(dataSource).withTableName(getTableName())
	...
	private void createFieldsDefinition(Class<?> c, FieldsDefinition parentFieldsDefinition) {
		...
		for (Field f : Utils.getAllFields(c)) {
			GrwaJdbcColumn j = f.getAnnotation(GrwaJdbcColumn.class);
			if (j != null) {
				fieldList.add(f);
			}
		}
		fieldList.sort(new Comparator<Field>() {
			...
			public int compare(Field o1, Field o2) {
				GrwaJdbcColumn j1 = o1.getAnnotation(GrwaJdbcColumn.class);
				GrwaJdbcColumn j2 = o2.getAnnotation(GrwaJdbcColumn.class);
	...
	private void fillValueDefinitionList() {
		...
		for (FieldsDefinition fd : fieldsDefinitionList) {
			if (fd.getJdbcColumn().keyValue() == GrwaJdbcColumn.VALUE) {
	...
	private void fillKeyDefinitionList() {
		...
		for (FieldsDefinition fd : fieldsDefinitionList) {
			if (fd.getJdbcColumn().keyValue() == GrwaJdbcColumn.KEY) {
